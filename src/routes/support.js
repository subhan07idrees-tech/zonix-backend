...
/**
 * Submit In-App Customer Support Ticket / Bug Report
 * POST /api/support/ticket
 */
router.post('/ticket', async (req, res) => {
  const { subject, message, telemetry } = req.body;
  if (!subject || !message) {
  ...
  try {
    ...
    const orgName = req.user?.orgName || 'DAT Fleet Org';
    ...
    if (result.success) {
      res.json({ success: true, message: 'Support ticket submitted successfully to support.zonix@gmail.com' });
    } else {
      res.status(500).json({ error: result.error || 'Failed to deliver support ticket' });
    }
  } catch (err) {
...
router.post('/broadcast', async (req, res) => {
  if (req.user?.role !== 'SUPER_ADMIN') {
    return res.status(403).json({ error: 'Only Super Admin can send mass broadcasts' });
  }
